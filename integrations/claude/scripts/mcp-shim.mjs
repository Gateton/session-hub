 ...
 * MCP shim.
 *
 * Claude Code copies a marketplace plugin into its own cache, so `.mcp.json`
 * cannot point at a path inside the session-hub checkout: the plugin has to find
 * the hub the same way its hooks do, with the shared resolver. This file is that
 * one line of glue. It holds no protocol logic of its own when the hub is there:
 * it hands the real server the same stdio Claude Code handed it.
 *
 * When the hub cannot be found anywhere, exiting silently would leave three
 * tools that never answer. Instead the shim speaks just enough JSON-RPC to
 * list the tools and to fail every call with the sentence that says how to fix
 * the installation. That keeps the failure visible where the user will see it:
 * in the answer to their question.
 */
